...
(function (global) {
  ...
  global.SheetClip = {
     ...
     parse2 : function (strData, strDelimiter) {
    ...
    parse: function (str) {  // the orginal
      ...
      // start here experiment
      var detect = {};
      detect.tab = countChar(rows[0],'\t');
      detect.comma = countChar(rows[0],',');
      detect.colon = countChar(rows[0],':');
      detect.semi = countChar(rows[0],';');
      detect.pipe = countChar(rows[0],'|');
      if(rows.length>1) { // swap delimiter to comma if no tabs and a comma.
         if     (detect.tab >=detect.comma && detect.tab >=detect.pipe && detect.tab >=detect.semi && detect.tab>=detect.colon) delim = "\t";
         else if(detect.semi >detect.comma && detect.semi >detect.pipe && detect.semi >detect.tab  && detect.semi>detect.colon) delim = ";";
         else if(detect.colon>detect.comma && detect.colon>detect.pipe && detect.colon>detect.tab  && detect.colon>detect.semi) delim = ":";
         else if(detect.pipe >detect.comma && detect.pipe >detect.semi && detect.pipe >detect.tab  && detect.pipe>detect.colon) delim = "|";
         else if(detect.comma>detect.tab   && detect.comma>detect.pipe && detect.comma>detect.semi && detect.comma>detect.colon)delim = ",";
         if (detect.tab == 0 && detect.comma == 0 && detect.pipe == 0 && detect.colon == 0 && detect.semi == 0 && detect.space > 0)delim = " ";
      }
      return global.SheetClip.parse2(str,delim);
      // end here experiment

      for (r = 0, rlen = rows.length; r < rlen; r += 1) {
        rows[r] = rows[r].split(delim);
...
function HandsontableColumnSorting() {
  ...
  this.sort = function () {
    ...
    var colMeta = instance.getCellMeta(0, instance.sortColumn+colOffset); // fixed bug here missing +colOffset
    //alert(instance.sortColumn+"/"+colOffset+"/"+colMeta.type);
    var sortFunction;
    switch (colMeta.type) {
      case 'date':
