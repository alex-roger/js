...
function InitializeGeocoder() {
	...
	extra_fields = ['address','city','state','zip','country']; // ['sw_corner','ne_corner'];
	...
	if ($('google_key_box') && $('google_key_box').value == '' && google_key.match(/\w+/)) { $('google_key_box').value = google_key; }
	if ($('bing_key_box') && $('bing_key_box').value == '' && bing_key.match(/\w+/)) { $('bing_key_box').value = bing_key; }
	if ($('mapquest_key_box') && $('mapquest_key_box').value == '' && mapquest_key.match(/\w+/)) { $('mapquest_key_box').value = mapquest_key; }
	if ($('mapquestopen_key_box') && $('mapquestopen_key_box').value == '' && mapquestopen_key.match(/\w+/)) { $('mapquestopen_key_box').value = mapquest_key; }
}
...
function ChangeSource() {
	...
	if ($(source+'_map_div')) { $(source+'_map_div').style.display = 'block'; }
	...
	if (source == 'google' && $('google_map_iframe')) {
		if (!self.google_map_iframe_centered && $('google_map_div') && $('google_map_div').style.display == 'block') {
...
function DetectDataType(input_id) {
	...
	var detected_type;
	if (first_row.match(/(^|\t|,|\|) *(street|add?ress?e?1?|city|county|town|plaats|ville|state|province|zip.?(code)?|postal|posta?l?.?code|code.?postale?|airport|latitude|longitude|lat|long?)\b/i)) {
		detected_type = 'table';
	} else {
		detected_type = 'list';
	}
	...
	for (var j=0; j<input_lines.length; j++) {
		...
		if (elevation) {
			...
			if (input_field_index['address'] != null && parts[input_field_index['address']]) {
				...
				location += ', '+parts[input_field_index['number']];
			}
			...
			if (input_field_index['county'] != null && parts[input_field_index['county']]) {
				if (input_field_index['zip'] == null || (input_field_index['zip'] != null && !parts[input_field_index['zip']])) {
					location += ', '+parts[input_field_index['county']]; // do not add county if there's ZIP info; it confuses some geocoders
				}
			}
...
function GeocodeMultiple(input_id,output_id) {
	...
	if (source == 'google' && !self.google_code_loaded) {
	...
	if (input_type == 'list' && input_lines[0].match(/(^|\t|,) *(street|add?ress?e?1?|city|county|town|ville|plaats|state|province|zip.?(code)?|postal|posta?l?.?code|p\..?code|code.?postale?|airport|latitude|longitude|lat|long?)\b/i)) {
	...
	} else if (input_type == 'table' && !input_lines[0].match(/(^|\t|,) *(street|add?ress?e?1?|city|county|town|ville|plaats|state|province|zip.?(code)?|postal|posta?l?.?code|p\..?code|code.?postale?|airport|latitude|longitude|lat|long?)\b/i)) {
	...
	if (self.absolute_limit && input_count > absolute_limit) {
...
function GeocodeLocation() {
	if (elevation) {
		if (source == 'google') {
			var coords = input_locations[location_counter].split(/\s*[\t;,\|]\s*/);
		...
		if (source == 'google') {
			...
			var loc = input_locations[location_counter].replace(/\t/g,', ');
			var url = si[source].base_url;
			url = url.replace(/{KEY}/g,geocoding_key).replace(/{QUERY}/g,URIEscape_Lite(loc));
		...
		if (elevation) {
			...
			if (source == 'bing') {
				...
				if (status && status == google.maps.GeocoderStatus.OK && results && results[0] && results[0].geometry && results[0].geometry.location) {
					...
					d.gv_name = input_locations[location_counter].replace(/\t/g,', ');
			...
			if (!d.gv_name) { d.gv_name = input_locations[location_counter].replace(/\t/g,', '); }
		} // end if(elevation)
	}
...
function OutputRow(d) {
	if (!input_lines[location_counter].match(/\w/)) { // blank line in input -> blank line in output
		...
		if (source == 'google' && $('google_map_iframe') && (lat != 0 || lon != 0)) {
...
function ClearResultsBoxAndMap () {
	...
	$('progress').innerHTML = '';
	if ($('google_map_div') && $('google_map_div').style.display != 'none' && $('google_map_iframe')) {
...
function KeyExplanation() {
	...
	alert('Each '+si[source].name+' key is only allowed a limited number of lookups per day. If you want to use this form to perform more than '+keyless_limit+' lookup'+plural+' at a time, you need to enter your own key.')
}
